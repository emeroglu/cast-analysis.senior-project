...
<head>
    ...
    <script type="text/javascript">
        ...
        var appReady = false,googleProfileId = "";

        var sessions = [],locationBased = [];
        ...
        .controller("Ctrl", function ($scope, $http) {
            ...
            $scope.locationBased = function () {

                gapi.client.analytics.data.ga.get({
                    'ids': 'ga:' + googleProfileId,
                    'start-date': '2015-01-01',
                    'end-date': '2016-12-30',
                    'metrics': 'ga:pageviews',
                    'dimensions': 'ga:city,ga:deviceCategory',
                ...
                .then(function (response) {
                    ...
                    $scope.header = "Location Based Data";

                    locationBased = response.result.rows;

                    console.log(JSON.stringify(sessions));

                    Reorganize_ForLocationBased("desktop");
                    Graph_LocationBased("#desktopChart", "blue", "Total Desktop Views");

                    Reorganize_ForLocationBased("mobile");
                    Graph_LocationBased("#mobileChart", "green", "Total Mobile Views");
    ...
    <script>
        ...
        function Reorganize_ForLocationBased(type) {

            data = [];

            var index = 0;

            for (var i = 0; i < locationBased.length; i++) {

                if (locationBased[i][1] == type) {
                    index++;

                    data.push([index, parseInt(locationBased[i][2])]);

                    cities.push([index, locationBased[i][0]]);
                }
        ...
        function Graph_LocationBased(id, color, caption) {
